...
<p>
  Gold has occupied a unique position in global financial markets for millennia. Unlike equities or fixed income,
  gold generates no cash flows, pays no dividends, and has no earnings to discount. Its value derives almost entirely
  from its role as a store of value, a monetary reserve asset, and &mdash; critically for this study &mdash; a safe-haven
  instrument to which capital flows during periods of elevated geopolitical uncertainty. The question of whether
  geopolitical risk systematically drives gold returns is both practically important for portfolio construction and
  theoretically interesting for understanding how tail-risk expectations are priced across asset classes.
</p>

<p>
  The challenge in studying geopolitical risk has historically been measurement. Unlike inflation, interest rates, or
  volatility, geopolitical risk is latent &mdash; it cannot be directly observed from market prices. Caldara and
...
<p>
  ...
  returns across approximately 490 monthly observations spanning 1985 to 2026. We test whether GPR <em>levels</em>
...
<p>
  Our central finding is a paradox: geopolitical risk clearly conditions gold return distributions &mdash; high-GPR
  months exhibit elevated volatility, positive skew, and modestly higher mean returns &mdash; but the directional
  predictive power is negligible. The GPR index tells you about the <em>environment</em> gold is trading in, but
  not about the <em>direction</em> gold will move. This distinction has direct implications for how geopolitical
...
<p>
  The academic literature on gold as a safe haven is extensive. Baur and Lucey (2010) established the distinction
  ...
  property is short-lived &mdash; typically lasting around 15 trading days after a shock. Baur and McDermott (2010)
  extended this analysis to a broader set of international equity markets, confirming gold's safe-haven role for
...
<p>
  ...
  tail dependence. This dollar-gold nexus is important for our study because geopolitical shocks often coincide
  with dollar movements, creating a potential confounding channel.
</p>

<p>
  The direct link between geopolitical risk and gold was facilitated by Caldara and Iacoviello (2022), whose
  GPR index provided the first systematic, newspaper-based measure of geopolitical risk with consistent monthly
  coverage from 1985. Their original paper documented that GPR shocks reduce real activity, lower stock returns,
  and increase capital flows to safe assets. However, the magnitude and persistence of the gold response to GPR
...
<p>
  ...
  early 2026, capturing recent geopolitical episodes including the Russia-Ukraine conflict (2022&ndash;present),
  the Hamas-Israel conflict (2023&ndash;present), and ongoing US-China tensions. Second, we test the relationship
  not only at the monthly level but also at intraday horizons (1 to 60 minutes) using a high-frequency GPR
  sentiment proxy, with proper walk-forward methodology to avoid look-ahead bias. Third, we conduct formal
...
<p>
  ...
  beginning of war, escalation of war, and terrorist acts. The index is normalised to have a mean of 100
  over the 2000&ndash;2009 baseline period.
</p>

<p>
  We use the monthly GPR series from January 1985 through early 2026, providing approximately 490 observations.
  The index exhibits substantial right skew: the median is approximately 85, but values exceeding 300 occur
  during major events (the Gulf War in 1990&ndash;91, the September 11 attacks in 2001, the Iraq War in 2003,
...
<p>
  ...
  yielding approximately 96 months for the joint monthly analysis.
</p>
...
<p>
  ...
  This produces approximately 13,700 aligned minute-bar observations. The intraday GPR signal is transformed
...
<p>
  Additionally, we identify <strong>Spike</strong> months as those exceeding the 95th percentile &mdash;
  these correspond to acute geopolitical crises. For each regime, we compute the distribution of gold returns
...
<p>
  Rather than using the GPR level, which is persistent, we also examine GPR <em>changes</em> &mdash; the
  month-over-month percentage change in the index. These are divided into quintiles:
</p>

<ul>
  <li><strong>Q1</strong> (Big Drop): largest GPR declines &mdash; geopolitical tensions easing</li>
  ...
  <li><strong>Q5</strong> (Big Spike): largest GPR increases &mdash; acute geopolitical escalation</li>
</ul>

<p>
  For each quintile, we compute the same-month gold return, forward returns at 1-month, 3-month, and
  6-month horizons, and win rates at each horizon. The quintile approach has the advantage of being
  distribution-free and robust to outliers in the GPR change series.
</p>

<h3>4.3 Predictive Testing Framework</h3>

<p>
  To assess whether GPR has genuine predictive power for gold direction, we employ two complementary
  approaches:
</p>

<p>
  <strong>Spearman rank correlation:</strong> We compute the Spearman correlation between GPR features
  (level and MoM change) and gold returns at various horizons. Spearman correlation is preferred over
  Pearson because it does not assume linearity and is robust to outliers in both variables.
</p>

<p>
  <strong>Walk-forward logistic regression:</strong> We fit a logistic regression model on a 70% training
...
<p>
  ...
  The difference in AUC between the two models quantifies the marginal information content of GPR.
</p>
...
<p>
  ...
  minutes, the win rate, and a binomial test against the null hypothesis that the win rate equals 50%.
</p>
...
<p>
  The regime analysis reveals a clear pattern in return <em>distributions</em> even if mean returns are
  similar. High-GPR months show wider dispersion in gold returns compared to Low-GPR months. The
  interquartile range is approximately 30&ndash;40% wider in the High-GPR regime. This is consistent
  with the safe-haven hypothesis: during geopolitical stress, gold becomes more reactive to news,
  producing both large rallies (flight-to-safety flows) and sharp reversals (risk appetite recovery).
</p>

<div class="finding-box">
  <strong>Finding 1:</strong> High-GPR regimes increase gold return dispersion but do not reliably shift
  the mean. The volatility of gold returns is approximately 30&ndash;40% higher in High-GPR months relative
...
<p>
  The quintile analysis examines whether the <em>direction</em> and <em>magnitude</em> of GPR changes predict
  gold returns. If geopolitical escalation drives safe-haven flows, we would expect Q5 (big GPR spikes) to
...
<p>
  The win-rate analysis provides a cleaner read on directional predictability. Across all horizons, the
  variation in win rates across quintiles is modest. No quintile consistently achieves a win rate above 60%
  or below 40% at any horizon. The Q5 (big spike) win rate fluctuates around the 50&ndash;55% range, which
  is weakly positive but not statistically significant given sample sizes of approximately 19 observations
  per quintile.
</p>
...
<p>
  A critical question for any empirical relationship is whether it is stable over time or driven by a
  few extreme episodes. Figure 6 addresses this by colouring observations by sub-period. The scatter
  is diffuse in all periods, with no clear clustering pattern. The OLS regression line is nearly flat,
  and the $R^2$ is negligible. Importantly, the relationship does not appear to strengthen during
  crisis periods (2020&ndash;2021, coloured red) compared to calmer periods (2018&ndash;2020, coloured blue),
...
<p>
  ...
  The 12-month rolling Spearman correlation oscillates between approximately &minus;0.6 and +0.6, spending
  roughly equal time in positive and negative territory. There are extended periods &mdash; sometimes
  lasting 12 to 18 months &mdash; where the correlation is persistently negative, meaning gold <em>falls</em>
  when geopolitical risk rises. This sign-switching behaviour is a hallmark of a non-stationary, regime-dependent
...
<p>
  The lag sweep analysis tests the Spearman correlation between the 1-bar GPR log difference and future
  gold returns at lags from 1 to 120 minutes. The results are summarised in the following table:
</p>
...
<p>
  At short horizons (1&ndash;15 minutes), the Spearman correlation is essentially zero and statistically
  insignificant. At longer horizons (30&ndash;120 minutes), small negative correlations emerge that achieve
  statistical significance ($p < 0.05$), but the effect sizes are tiny: $|\\rho| < 0.03$ throughout.
  The oriented AUC (taking the best of AUC and 1&minus;AUC) peaks at 0.514 at the 120-minute lag &mdash;
  barely distinguishable from the 0.500 baseline of a random classifier.
</p>

<p>
  The <em>negative</em> sign of the correlation at longer horizons is noteworthy. It implies that GPR
  increases are followed by gold <em>declines</em> rather than rallies, which contradicts the naive
  safe-haven narrative. One interpretation is mean-reversion: GPR spikes trigger an initial safe-haven
  rally (within minutes), but the subsequent price action over 30&ndash;120 minutes partially reverses as
...
<p>
  ...
  horizons, the AUC slightly exceeds 0.50, but the binomial test p-values (0.130 and 0.251) fail to reject
  the null hypothesis that accuracy equals 50%. Only the 30-minute horizon achieves a nominally significant
  ...
  indicating that the model is worse than a coin flip &mdash; a sign of overfitting to the training set.
</p>

<h3>8.3 Ablation: Price-Only vs Price + GPR</h3>

<p>
  The most informative test is the ablation: does adding GPR features to a price-only model improve
...
<p>
  ...
  largest degradation (&minus;0.032) at the 30-minute horizon. Only at the 60-minute horizon does GPR
  provide a marginal improvement (+0.012), but this is on a model that is already performing below
  chance (AUC = 0.478 baseline). The GPR features appear to inject noise into the logistic regression
...
<p>
  If GPR has any predictive value for gold, it is most likely during extreme spikes &mdash; the tail events
  that dominate the index's variation. We isolate observations where the 1-bar GPR log difference exceeds
  the 95th percentile (approximately 687 events) and the 99th percentile (approximately 138 events), and
...
<p>
  ...
  below-baseline win rates. At the 15-minute horizon, the top 1% of GPR spikes show a positive mean return
  of +4.08 bps, but the win rate (51.5%) is below the baseline (53.4%) and the binomial test is far from
...
<p>
  The disconnect between the intuitive appeal of the GPR-gold link and its empirical weakness as a directional
  signal can be understood through several mechanisms:
</p>
...
<p>
  <strong>Non-linear and asymmetric effects.</strong> The GPR-gold relationship may be non-linear &mdash;
  only the most extreme GPR events (wars, invasions, terrorist attacks) trigger sustained safe-haven flows,
  while moderate GPR fluctuations are noise. But even in our spike-conditional analysis (Finding 5), extreme
  GPR events fail to produce significant directional effects, suggesting that the non-linearity, if it
  exists, operates at a frequency lower than our monthly or minute-level analysis can capture.
</p>

<p>
  <strong>Regime dependence.</strong> The rolling correlation analysis (Finding 3) shows that the GPR-gold
  relationship changes sign over multi-year horizons. In some regimes, geopolitical risk drives capital into
  ...
  nature of the geopolitical shock &mdash; variables that the GPR index does not capture.
</p>

<h3>9.2 Practical Implications for Systematic Trading</h3>

<p>
  For systematic gold trading models, our results suggest the following:
</p>
...
<ul>
  ...
  <li>
    <strong>GPR may have value as a volatility regime filter.</strong> High-GPR regimes exhibit wider return
  ...
  <li>
    ...
    aggregates heterogeneous events (wars, terrorism, military buildups, diplomatic tensions) into a single
    number. Different event types may have very different implications for gold. A more granular approach &mdash;
    using the GPR sub-indices for threats vs. acts, or building event-specific features from news data &mdash;
...
<p>
  ...
  is short-lived (approximately 15 trading days). The GPR index, which measures the <em>level</em> of
  geopolitical risk rather than the <em>shock</em> component, would not capture such transient effects.
  Our finding that GPR changes (rather than levels) have slightly more information is consistent with
...
<p>
  ...
  approximately 40 years of monthly data and at intraday frequency using a live GPR proxy. Five findings
  emerge:
</p>

<ol>
  <li>
    High-GPR regimes increase gold return dispersion by 30&ndash;40% but do not reliably increase mean returns.
  </li>
  <li>
...
<p>
  The practical implication is clear: the GPR index should not be used as a directional trading signal for
