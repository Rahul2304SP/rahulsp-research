...
// /api/homefinder-uk against the HOMEFINDER_KEY secret (never shipped to the client).
...
export default function HomeFinder() {
  ...
  async function requestRefresh() {
    const k = sessionStorage.getItem("hfuk_key") || key;
    setRefreshing(true); setRefreshMsg("");
    try {
      const r = await fetch("/api/homefinder-uk", {
        method: "POST",
  ...
  async function load(k: string) {
    ...
    try {
      const r = await fetch("/api/homefinder-uk", { headers: { "x-homefinder-key": k } });
      ...
      sessionStorage.setItem("hfuk_key", k);
      setReport(data);
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem("hfuk_key");
    ...
    <main style={wrap}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => { sessionStorage.removeItem("hfuk_key"); setReport(null); setKey(""); }}
